...
// Static files (served at /canchas)
...
// ==================== SPORTS ====================
app.get(`${BASE}/api/sports`, (req, res) => {
  res.json(SPORTS);
});

// ==================== COURTS ====================
app.get(`${BASE}/api/courts`, (req, res) => {
...
app.get(`${BASE}/api/courts/:id`, (req, res) => {
  ...
  res.json({ ...court, sport_info: SPORTS.find(s => s.id === court.sport) || { name: court.sport, emoji: '🏟️' } });
});

app.post(`${BASE}/api/courts`, (req, res) => {
  const { name, sport, location, price_hour } = req.body;
  if (!name || !sport) return res.status(400).json({ error: 'Nombre y deporte son requeridos' });

  const result = db.prepare('INSERT INTO courts (name, sport, location, price_hour) VALUES (?, ?, ?, ?)')
    .run(name, sport, location || '', price_hour || 0);
  res.status(201).json(db.prepare('SELECT * FROM courts WHERE id = ?').get(result.lastInsertRowid));
});

app.put(`${BASE}/api/courts/:id`, (req, res) => {
  ...
  db.prepare(`UPDATE courts SET name=?, sport=?, location=?, price_hour=?, active=? WHERE id=?`)
    .run(name??existing.name, sport??existing.sport, location??existing.location,
         price_hour??existing.price_hour, active??existing.active, req.params.id);
  res.json(db.prepare('SELECT * FROM courts WHERE id = ?').get(req.params.id));
});

app.delete(`${BASE}/api/courts/:id`, (req, res) => {
  db.prepare('DELETE FROM courts WHERE id = ?').run(req.params.id);
  res.json({ success: true });
});

// ==================== SLOTS ====================
app.get(`${BASE}/api/courts/:id/slots`, (req, res) => {
  ...
  const bookings = db.prepare('SELECT time FROM bookings WHERE court_id = ? AND date = ?')
...
app.post(`${BASE}/api/bookings`, (req, res) => {
  ...
  const existing = db.prepare('SELECT * FROM bookings WHERE court_id = ? AND date = ? AND time = ?')
    .get(court_id, date, time);
  if (existing) return res.status(409).json({ error: 'Horario ya reservado' });

  const result = db.prepare('INSERT INTO bookings (court_id, date, time, client_name, client_phone) VALUES (?, ?, ?, ?, ?)')
...
// ==================== GAMES ====================
app.get(`${BASE}/api/games`, (req, res) => {
  const { court_id, date } = req.query;
  let query = `SELECT g.*, c.name as court_name, c.sport, c.location,
    (SELECT COUNT(*) FROM game_participants WHERE game_id = g.id) as participants_count
    FROM games g JOIN courts c ON g.court_id = c.id WHERE 1=1`;
  const params = [];
...
app.post(`${BASE}/api/games`, (req, res) => {
  ...
  const result = db.prepare('INSERT INTO games (court_id, date, time, players_needed, title) VALUES (?, ?, ?, ?, ?)')
...
app.post(`${BASE}/api/games/:id/join`, (req, res) => {
  ...
  const currentCount = db.prepare('SELECT COUNT(*) as count FROM game_participants WHERE game_id = ?').get(gameId).count;
  if (currentCount >= game.players_needed) return res.status(409).json({ error: 'El juego ya está completo' });

  const result = db.prepare('INSERT INTO game_participants (game_id, name, phone) VALUES (?, ?, ?)')
    .run(gameId, name, phone);
  res.status(201).json({ id: result.lastInsertRowid, game_id: gameId, name, phone });
});

app.delete(`${BASE}/api/games/:id/leave`, (req, res) => {
  const { phone } = req.body;
  const gameId = req.params.id;
  const participant = db.prepare('SELECT * FROM game_participants WHERE game_id = ? AND phone = ?').get(gameId, phone);
...
app.get(`${BASE}/api/games/:id/participants`, (req, res) => {
  res.json(db.prepare('SELECT * FROM game_participants WHERE game_id = ? ORDER BY created_at ASC').all(req.params.id));
});
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`[Server] Canchas MVP running on http://0.0.0.0:${PORT}/canchas`);
});
